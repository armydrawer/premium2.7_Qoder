...
$h_change = get_theme_option('h_change', array('fixheader', 'linkhead', 'phone', 'icq', 'skype', 'email', 'telegram', 'viber', 'whatsapp', 'jabber', 'hideloginbutton'));
...
<div id="container">
	...
	<div class="container">
		...
		<div class="mobile_menu_wrap">
			...
			<div class="mobile_menu">
				<div class="mobile_menu_ins">
					<div class="mobile_menu_title"><?php _e('Menu', 'pntheme'); ?></div>
		...
		<!-- top bar -->
		<div class="topbar_wrap <?php if (1 == $h_change['fixheader']) { ?>fix_div<?php } ?>" fix-top="0" fix-id="fix_topbar">
			<div class="topbar_ins" id="fix_topbar">
				<div class="topbar">
					<div class="topbar_contain">
							...
							<div class="topbar_icon_tabul">
								...
								<?php if ($h_change['telegram']) { ?>
									<div class="topbar_icon telegram">
										<?php echo get_contact($h_change['telegram'], 'tm', __('to write', 'pntheme')); ?>
									</div>		
								<?php } ?>
						...
						<?php if (1 != $hideloginbutton) { ?>
							<?php if ($user_id) { ?>
								<a href="<?php echo get_pn_action('logout', 'get'); ?>" class="toplink toplink_exit"><?php _e('Exit', 'pntheme'); ?></a>
								<a href="<?php echo $plugin->get_page('account'); ?>" class="toplink toplink_userlogin"><?php _e('Account', 'pntheme'); ?></a>
							<?php } else { ?>
								<a href="<?php echo $plugin->get_page('register'); ?>" class="toplink toplink_signup js_window_join"><?php _e('Sign up', 'pntheme'); ?></a>
								<a href="<?php echo $plugin->get_page('login'); ?>" class="toplink toplink_signin js_window_login"><?php _e('Sign in', 'pntheme'); ?></a>
							<?php } ?>
						<?php } ?>
		...
		<!-- top menu -->
		<div class="tophead_wrap <?php if (2 == $h_change['fixheader']) { ?>fix_div<?php } ?>" fix-top="0" fix-id="fix_menu">
			<div class="tophead_ins" id="fix_menu">
				<div class="tophead">
					
					<div class="logoblock">
						<div class="logoblock_ins">
							<?php if (1 == $h_change['linkhead'] and !is_front_page() or 1 != $h_change['linkhead']) { ?>
