...
if (is_admin()) {
	...
	add_filter('pn_adminpage_title_pn_add_bcorrs', 'def_adminpage_title_pn_add_bcorrs');
	function def_adminpage_title_pn_add_bcorrs() {
	...
	add_action('pn_adminpage_content_pn_add_bcorrs', 'def_adminpage_content_pn_add_bcorrs');
	function def_adminpage_content_pn_add_bcorrs() {
		global $wpdb;
		...
		if ($id) {
			$data = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_directions WHERE id = '$id'");
			if (isset($data->id)) {
				$data_id = $data->id;
			}	
		}
		...
		$back_menu['back'] = array(
			'link' => admin_url('admin.php?page=pn_bcorrs'),
		...
		if ($data_id) {
			$back_menu['add'] = array(
				'link' => admin_url('admin.php?page=pn_add_bcorrs'),
				'title' => __('Add new', 'pn')
			);	
		}
		...
		foreach ($directions as $direction) { 
			$opts[$direction->id] = pn_strip_input($direction->tech_name) . pn_item_status($direction, 'direction_status', array('0' => __('inactive direction', 'pn'), '2' => __('hold direction', 'pn'))) . pn_item_basket($direction);
		}
		...
		$alls = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_cities ORDER BY city_title ASC");
		$cities[0] = '--' . __('No item', 'pn') . '--';
		foreach ($alls as $all) {
			$cities[$all->city_id] = pn_strip_input($all->city_title);
		}	
		$options['city_id'] = array(
			'view' => 'select_search',
			'title' => __('City', 'pn'),
			'options' => $cities,
			'default' => is_isset($data, 'city_id'),
			'name' => 'city_id',
		);		
		$options['v1'] = array(
			'view' => 'select_search',
		...
		$options['show_positions'] = array(
			'view' => 'textfield',
			'default' => '<a href="#" class="button js_displayrating_bestchangeapi" target="_blank">' . __('Show rating', 'pn') . '</a>',
		);		
		$options['line1'] = array(
			'view' => 'line',
		);		
		...
		$options['float_course'] = array(
			'view' => 'select',
			'title' => __('Float course', 'pn'),
			'options' => array('0' => __('auto', 'pn'), '1' => __('1 = XXX', 'pn'), '2' => __('XXX = 1', 'pn')),
			'default' => is_isset($data, 'float_course'),
			'name' => 'float_course',
		);		
		$options['convert_course'] = array(
			'view' => 'select',
			'title' => __('Convert course to', 'pn'),
			'options' => array('0' => __('No', 'pn'), '1' => __('1 = XXX', 'pn'), '2' => __('XXX = 1', 'pn')),
			'default' => is_isset($data, 'convert_course'),
			'name' => 'convert_course',
		);		
		$options['line4'] = array(
			'view' => 'line',
...
jQuery(function($) {
	...
	function set_show_link() {
		var url = '<?php echo get_request_link('displayrating_bestchangeapi', 'html'); ?>?v1=' + $('#pn_v1').val() + '&v2=' + $('#pn_v2').val() + '&city_id=' + $('#pn_city_id').val() + '&minres=' + $('#pn_min_res').val();
		$('.js_displayrating_bestchangeapi').attr('href', url);
	}
	$(document).on('change', '#pn_city_id, #pn_v1, #pn_v2, #pn_min_res', function() {
		set_show_link();
	});
	set_show_link();
...
add_action('premium_action_pn_add_bcorrs', 'def_premium_action_pn_add_bcorrs'); 
function def_premium_action_pn_add_bcorrs() {
	global $wpdb;	
	...
	$last_data = '';
	if ($data_id > 0) {
		$last_data = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_directions WHERE id = '$data_id'");
	...
	$v1 = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_currency_codes WHERE currency_code_id = '$id_v1'");
	$v2 = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_currency_codes WHERE currency_code_id = '$id_v2'");
	if (!isset($v1->id) or !isset($v2->id)) {
		$form->error_form(__('Error! You have not specified the Send or Receive value', 'pn'));
	}
		
	$city_id = intval(is_param_post('city_id'));
	$array['city_id'] = $city_id;
		
	if ($city_id > 0) {
		$city = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_cities WHERE city_id = '$city_id'");
		if (!isset($city->id)) {
			$form->error_form(__('Error! You have not specified the city value', 'pn'));
		}	
	}
	...
	$array['convert_course'] = intval(is_param_post('convert_course'));
	$array['float_course'] = intval(is_param_post('float_course'));
	...
	$array = apply_filters('pn_bcorrs_addform_post', $array, $last_data);		
				
	if ($data_id) {	
		$res = apply_filters('item_bcorrs_edit_before', pn_ind(), $data_id, $array, $last_data);
		if ($res['ind']) {
			$result = $wpdb->update($wpdb->prefix . 'bestchangeapi_directions', $array, array('id' => $data_id));
				
			$res_errors = _debug_table_from_db($result, 'bestchangeapi_directions', $array);
			_display_db_table_error($form, $res_errors);	
				
			if (isset($last_data->direction_id)) {
				if ($last_data->direction_id != $direction_id) {
					$wpdb->update($wpdb->prefix . "directions", array('bestchangeapi_id' => 0), array('id' => $last_data->direction_id));
				}
			}
			$wpdb->update($wpdb->prefix . "directions", array('bestchangeapi_id' => 1), array('id' => $direction_id));
				
			do_action('item_bcorrs_edit', $data_id, $array, $last_data, $result, $direction_id, $direction);
		} else { $form->error_form(is_isset($res, 'error')); }
	} else {
		$res = apply_filters('item_bcorrs_add_before', pn_ind(), $array);
		if ($res['ind']) {
			$result = $wpdb->insert($wpdb->prefix . 'bestchangeapi_directions', $array);
			$data_id = $wpdb->insert_id;
			if ($result) {	
				$wpdb->update($wpdb->prefix . "directions", array('bestchangeapi_id' => 1), array('id' => $direction_id));
				do_action('item_bcorrs_add', $data_id, $array, $direction_id, $direction);	
			} else {
				$res_errors = _debug_table_from_db($result, 'bestchangeapi_directions', $array);
				_display_db_table_error($form, $res_errors);
			}
	...
	$url = admin_url('admin.php?page=pn_add_bcorrs&item_id=' . $data_id . '&reply=true');
