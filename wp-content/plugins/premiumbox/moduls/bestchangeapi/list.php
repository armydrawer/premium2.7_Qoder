...
if (is_admin()) {
	...
	add_filter('pn_adminpage_title_pn_bcorrs', 'def_adminpage_title_pn_bcorrs');
	function def_adminpage_title_pn_bcorrs($title) {
		
		return __('Adjustments', 'pn');
	}

	add_action('pn_adminpage_content_pn_bcorrs', 'def_adminpage_content_pn_bcorrs');
	function def_adminpage_content_pn_bcorrs() {
...
add_action('premium_action_pn_bcorrs', 'def_premium_action_pn_bcorrs');
function def_premium_action_pn_bcorrs() {
	global $wpdb;	
			...
			foreach ($_POST['ids'] as $id) {
				...
				if (isset($item->id)) {
					...
					if (count($arr) > 0) {
						...
						$result = $wpdb->update($wpdb->prefix . "bestchangeapi_directions", $arr, array('id' => $id));
						do_action('item_bcorrs_save', $item->id, $item, $result, $arr);
		...
		if (isset($_POST['id']) and is_array($_POST['id'])) {				
			...
			if ('active' == $action) {		
				foreach ($_POST['id'] as $id) {
					$id = intval($id);
					$item = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_directions WHERE id = '$id' AND status != '1'");
					if (isset($item->id)) {
						$res = apply_filters('item_bcorrs_active_before', pn_ind(), $id, $item);
						if ($res['ind']) {
							$result = $wpdb->query("UPDATE " . $wpdb->prefix . "bestchangeapi_directions SET status = '1' WHERE id = '$id'");
							do_action('item_bcorrs_active', $id, $item, $result);
						}
					}
				}				
			}
			...
			if ('deactive' == $action) {		
				foreach ($_POST['id'] as $id) {
					$id = intval($id);
					$item = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_directions WHERE id = '$id' AND status != '0'");
					if (isset($item->id)) {
						$res = apply_filters('item_bcorrs_deactive_before', pn_ind(), $id, $item);
						if ($res['ind']) {
							$result = $wpdb->query("UPDATE " . $wpdb->prefix . "bestchangeapi_directions SET status = '0' WHERE id = '$id'");
							do_action('item_bcorrs_deactive', $id, $item, $result);
						}
					}
				}	
			}					
			...
			if ('delete' == $action) {		
				foreach ($_POST['id'] as $id) {
					...
					$item = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_directions WHERE id = '$id'");
					if (isset($item->id)) {
						$res = apply_filters('item_bcorrs_delete_before', pn_ind(), $id, $item);
						if ($res['ind']) {
							$result = $wpdb->query("DELETE FROM " . $wpdb->prefix . "bestchangeapi_directions WHERE id = '$id'");
							do_action('item_bcorrs_delete', $id, $item, $result);
						}
					}
				}			
			}
				
			do_action('pntable_bcorrs_action', $action, $_POST['id']);
			$arrs['reply'] = 'true';
		} 		
	}
...
class pn_bcorrs_Table_List extends PremiumTable {
	...
	function column_default($item, $column_name) {
		...
		if ('status' == $column_name) {	
			...
			$html = '<a href="' . get_request_link('displayrating_bestchangeapi', 'html'). '?v1=' . $item->v1 . '&v2=' . $item->v2 . '&city=' . $item->city_id . '" class="button" target="_blank">' . __('Link', 'pn') . '</a>';
	...
	function get_row_actions($item) {
		...
		$actions = array(
			'edit'      => '<a href="' . admin_url('admin.php?page=pn_add_bcorrs&item_id=' . $item->id) . '">'. __('Edit', 'pn') .'</a>',
	...
	function prepare_items() {
		...
		if (1 == $this->navi) {
			$this->total_items = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "bestchangeapi_directions WHERE id > 0 $where");
		}	
		$this->items = $wpdb->get_results("SELECT * $select_sql FROM " . $wpdb->prefix . "bestchangeapi_directions WHERE id > 0 $where ORDER BY $orderby $order LIMIT $offset , $per_page");  		
	}
		
	function extra_tablenav($which) {
		?>
		<a href="<?php echo admin_url('admin.php?page=pn_add_bcorrs'); ?>"><?php _e('Add new', 'pn'); ?></a>		
		<?php 
	} 	  
} 