	...
	if ($user_id and 1 == $loadhistory or current_user_can('administrator') or current_user_can('pn_archive')) {
		...
		if (is_array($datas)) {
			foreach ($datas as $item) {
				...
				$line .= is_sum(is_isset($arch, 'course_give')) . '' . is_site_value(is_isset($arch, 'currency_code_give')) . '=' . is_sum(is_isset($arch, 'course_get')) . '' . is_site_value(is_isset($arch, 'currency_code_get')) . ';';
				$line .= is_sum(is_isset($arch, 'sum1dc')) . ' ' . get_cptgn(pn_strip_input(ctv_ml(is_isset($arch, 'psys_give')))) . ' ' . is_site_value(is_isset($arch, 'currency_code_give')) . ';';
				$line .= is_sum(is_isset($arch, 'sum2c')) . ' ' . get_cptgn(pn_strip_input(ctv_ml(is_isset($arch, 'psys_get')))) . ' ' . is_site_value(is_isset($arch, 'currency_code_get')) . ';';
