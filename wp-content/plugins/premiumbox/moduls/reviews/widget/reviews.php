	...
	function reviews_register_widgets() {
		class pn_reviews_Widget extends WP_Widget { 
			...
			public function widget($args, $instance) {
				...
				if (strlen($title_widget) < 1) { $title_widget = __('Reviews', 'pn'); }
					...
					if (!trim($temps)) {
						...
						<div class="widget_reviews_line ' . $fclass . ' ' . $lclass . ' ' . $oddeven . '">
							...
							<div class="widget_reviews_date">' . get_pn_time( $item->review_date, $reviews_date_format) . '</div>
								<div class="clear"></div>
							<div class="widget_reviews_content">' . _span_break_words(wp_trim_words(pn_strip_input($item->review_text), 15)) . '</div>
						</div>		
						';	
					}
				...
				<div class="widget widget_reviews_div">
					<div class="widget_ins">
						...
						<div class="widget_reviews_more_wrap">
							<a href="[url]" class="widget_reviews_more"><span>'. __('All reviews', 'pn') .'</span></a>
								<div class="clear"></div>
						</div>
					</div>
				</div>
				...
				$widget = replace_tags($array, $widget);
				echo $widget;
			}
			...
			<?php
				do_action('reviews_widget_options', $instance, $this);
