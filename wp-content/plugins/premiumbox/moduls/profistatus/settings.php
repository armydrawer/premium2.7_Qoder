...
if (is_admin()) {
	...
	function def_adminpage_content_pn_profistatus() {
        ...
        $statused_n = $statused + ['auto'=>__('uncreated order', 'pn')];
		$options['settings5'] = array(
			'view' => 'user_func',
			'func_data' => array('list' => $statused_n, 'name' => 'bid_active', 'title' => $title),
			'func' => '_profistatus_option',
		);
	...
	<div class="premium_standart_line">
		...
		<div class="premium_stline_right"><div class="premium_stline_right_ins">
			<div class="premium_wrap_standart">
				...
				$scroll_lists = array();
				if (is_array($list)) {
                    $list = list_checks_top($list, $sett);
					foreach ($list as $key => $val) {
						$scroll_lists[] = array(
							'title' => $val,
							'checked' => in_array($key, $sett),
							'value' => $key,
						);
					}
				}
