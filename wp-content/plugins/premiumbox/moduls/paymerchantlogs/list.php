...
if (is_admin()) {
	...
	add_filter('pn_adminpage_title_pn_paymerchantlogs', 'def_adminpage_title_pn_paymerchantlogs');
	function def_adminpage_title_pn_paymerchantlogs() {
		
		return __('Automatic payouts log', 'pn');
	} 

	add_action('pn_adminpage_content_pn_paymerchantlogs', 'def_adminpage_content_pn_paymerchantlogs');
	function def_adminpage_content_pn_paymerchantlogs() {
...
function def_premium_action_pn_paymerchantlogs() {
	...
	if (isset($_POST['save'])) {
		...
		$wpdb->query("DELETE FROM " . $wpdb->prefix . "ap_logs");
		do_action('pntable_paymerchantlogs_deleteall');
		$arrs['reply'] = 'true';
...
class pn_paymerchantlogs_Table_List extends PremiumTable {
	...
	function get_search() {
		...
		$options = array();
		$options[0] = '--' . __('All', 'pn') . '--';
		$items = $wpdb->get_results("SELECT DISTINCT(m_id) FROM " . $wpdb->prefix . "ap_logs ORDER BY m_id ASC");  		
	...
	function prepare_items() {
		...
		if ($this->navi) {
			$this->total_items = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "ap_logs WHERE id > 0 $where");
		}
		$this->items = $wpdb->get_results("SELECT * $select_sql FROM " . $wpdb->prefix . "ap_logs WHERE id > 0 $where ORDER BY $orderby $order LIMIT $offset , $per_page");  	
