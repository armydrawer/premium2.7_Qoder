...
add_filter('list_tabs_direction', 'bestchange_list_tabs_direction', 11, 2); 
function bestchange_list_tabs_direction($list_tabs, $item) {
	
	if (current_user_can('administrator') or current_user_can('pn_bestchange')) {
		$tab_title = '';
		$bestchange_id = intval(is_isset($item, 'bestchange_id'));
		if ($bestchange_id > 0) {
			$tab_title = ' <span class="bgreen">*</span>';
		}	
		$new_list_tabs = array();
		$new_list_tabs['bestchange'] = __('BestChange parser', 'pn') . $tab_title;
 	...
 	?>	
		...
		<div class="add_tabs_line">
			<div class="add_tabs_single">
				...
				<div class="premium_wrap_standart">
					...
					<?php 
					$atts = array();
					$atts['id'] = 'bestchange_city_id';
					$option_data = array();
					$opts = array('0' => __('No item', 'pn'));
					foreach ($cities as $city) {
						$opts[$city->city_id] = pn_strip_input($city->city_title);
					}	
					$form->select_search('bestchange_city_id', $opts, $city_id, $atts, $option_data); 
					?>
		...
		<div class="add_tabs_line">
			<div class="add_tabs_single">
				...
				<div class="premium_wrap_standart">
					...
					<?php 
					$atts = array();
					$atts['id'] = 'bestchange_v1';
					$option_data = array();
					$opts = array('0' => '--' . __('No item', 'pn') . '--');
					foreach ($alls as $all) {
						$opts[$all->currency_code_id] = pn_strip_input($all->currency_code_title);
					}	
					$form->select_search('bestchange_v1', $opts, $v1, $atts, $option_data); 
					?>					
			...
			<div class="add_tabs_single">
				...
				<div class="premium_wrap_standart">
					...
					<?php 
					$atts = array();
					$atts['id'] = 'bestchange_v2';
					$option_data = array();
					$opts = array('0' => '--' . __('No item', 'pn') . '--');
					foreach ($alls as $all) {
						$opts[$all->currency_code_id] = pn_strip_input($all->currency_code_title);
					}	
					$form->select_search('bestchange_v2', $opts, $v2, $atts, $option_data); 
					?>					
		...
		<div class="add_tabs_line">
			<div class="add_tabs_single">
				...
				<div class="premium_wrap_standart">
					<select name="bestchange_float_course" autocomplete="off">
		...
		<div class="add_tabs_line">
			<div class="add_tabs_single">
				...
				<div class="premium_wrap_standart">
					<input type="text" name="bestchange_min_res" id="bestchange_min_res" style="width: 100%;" value="<?php echo is_sum(is_isset($broker, 'min_res')); ?>" />
				</div>
			</div>
			<div class="add_tabs_single">
				<div class="add_tabs_sublabel"><span><?php _e('Show rating', 'pn'); ?></span></div>	
				<div class="premium_wrap_standart">
					<a href="#" class="button js_displayrating_bestchange" target="_blank"><?php _e('Show rating', 'pn'); ?></a>
						<div class="clear"></div>
				</div>
			</div>			
		</div>		
		...
		<div class="add_tabs_line">
			<div class="add_tabs_single">
				<div class="add_tabs_sublabel"><span><?php echo __('Convert course to', 'pn'); ?></span></div>
				<div class="premium_wrap_standart">
					<select name="bestchange_convert_course" autocomplete="off">
...
jQuery(function($){
	...
	function set_show_link() {
		var url = '<?php echo get_request_link('displayrating_bestchange', 'html'); ?>?v1=' + $('#bestchange_v1').val() + '&v2=' + $('#bestchange_v2').val() + '&city_id=' + $('#bestchange_city_id').val() + '&minres=' + $('#bestchange_min_res').val();
		$('.js_displayrating_bestchange').attr('href', url);
	}
	$(document).on('change', '#bestchange_city_id, #bestchange_v1, #bestchange_v2, #bestchange_min_res', function() {
		set_show_link();
	});
	set_show_link();
		...
		if ($direction_id) {
			...
			if ($vid1 > 0 and $vid2 > 0) {
				...
				if (isset($v1->id) and isset($v2->id)) {
					...
					$arr['city_id'] = intval(is_param_post('bestchange_city_id'));
					...
					$arr['convert_course'] = intval(is_param_post('bestchange_convert_course'));
					$arr['float_course'] = intval(is_param_post('bestchange_float_course'));
					...
					if (isset($broker->id)) {
						$result = $wpdb->update($wpdb->prefix . "bestchange_directions", $arr, array('id' => $broker->id));
