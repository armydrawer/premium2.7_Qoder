...
add_filter('pn_adminpage_title_pn_live_bids', 'pn_admin_title_pn_live_bids');
function pn_admin_title_pn_live_bids() {
	
	return __('LIVE orders', 'pn');
} 

add_action('pn_adminpage_content_pn_live_bids', 'def_adminpage_content_pn_live_bids');
function def_adminpage_content_pn_live_bids() {
	global $wpdb;
...
<div class="head_bids_window">
	...
	<div class="change_div" id="live_change">
		<div class="change_div_ins">	
			...
			<div class="change_div_line">
				<div class="change_div_input">
					<label><input type="checkbox" name="hidetransit" id="the_hidetransit" <?php checked(1, $hidetransit); ?> autocomplete="off" value="1" /> <?php _e('hide orders since transition', 'pn'); ?></label>
				</div>
			</div>		
			...
			<div class="change_div_line">
				...
				<div class="change_div_input">
					...
					if (is_array($bid_status_list)) {
						...
						?>
						<div><label><input type="checkbox" name="status[]" class="checkbox_status" <?php if (in_array($key, $statused)) { ?>checked="checked"<?php } ?> autocomplete="off" value="<?php echo $key; ?>" /> <?php echo $status; ?></label></div>
						<?php }
					}
					?>
...
<?php
$ldata = $wpdb->get_row("SELECT id FROM " . $wpdb->prefix . "exchange_bids WHERE status != 'auto' ORDER BY id DESC LIMIT 1");
						...
						for (var i = 0; i < bids.length; i++) {
							...
							var temp_bid = '<div class="one_bids_live js_bids" id="bid_id' + data['id'] + '">' +
								...
								'<div class="obl_status">' +
									...
									'<div class="premium_clear"></div>'+
								'</div>' +
								'<a href="' + data['link'] + '" target="_blank" class="obl_view"><?php _e("View", "pn"); ?></a>' +
