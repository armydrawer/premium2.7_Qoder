...
if (is_admin()) {
	...
	add_filter('pn_adminpage_title_pn_export_bestchangeapi', 'pn_admin_title_pn_export_bestchangeapi');
	function pn_admin_title_pn_export_bestchangeapi($page){
		
		return __('bestchangeapi Export/Import', 'pn');
	} 

 	add_action('pn_adminpage_content_pn_export_bestchangeapi', 'def_pn_admin_content_pn_export_bestchangeapi');
	function def_pn_admin_content_pn_export_bestchangeapi() {
		global $wpdb;
		
		if (current_user_can('administrator') or current_user_can('pn_impexp_bestchange')) {
	?>
	<div class="premium_body">	
		<form method="post" target="_blank" action="<?php the_pn_link('export_bestchangeapi', 'post'); ?>">
	...
	<div class="premium_body">	
		<form method="post" target="_blank" action="<?php the_pn_link('import_bestchangeapi', 'post'); ?>" enctype="multipart/form-data">
			<div class="premium_standart_line"> 
	...
	add_action('premium_action_export_bestchangeapi', 'def_premium_action_export_bestchangeapi');
	function def_premium_action_export_bestchangeapi() {
		...
		$file = $path.'bestchangeapi_export-' . date('Y-m-d-H-i') . '.csv';           
		$fs = @fopen($file, 'w');
		...
		$array = array(
			...
			'bestchange' => __('bestchangeapi', 'pn'),
		...
		foreach ($items as $item) {
			$b_data = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "bestchangeapi_directions WHERE direction_id = '{$item->id}'");
	...
	add_action('premium_action_import_bestchangeapi', 'def_premium_action_import_bestchangeapi');
	function def_premium_action_import_bestchangeapi() {
		global $wpdb, $premiumbox;	
				...
				if ($_FILES["importfile"]["size"] > 0 and $_FILES["importfile"]["size"] < $max_upload_size) {
					...
					if (move_uploaded_file($tempFile, $targetFile)) {
										...
										if (count($db_array) > 0) {
											...
											if ($bestchange) {
												...
												if ($array['direction_id'] > 0) {
													...
													$wpdb->update($wpdb->prefix . "directions", array('bestchangeapi_id' => 1), array('id' => $array['direction_id']));
												}
											}
										}
									}	
						...
						if (0 == $error) {
							...
							$url = admin_url('admin.php?page=pn_export_bestchangeapi&reply=true');
							wp_redirect($url);
							exit;	
						}	
					} else {
