...
if (is_admin()) {
	...
	add_filter('pn_adminpage_title_all_add_usfield', 'def_adminpage_title_all_add_usfield');
	function def_adminpage_title_all_add_usfield() {
		
		$id = intval(is_param_get('item_id'));
		if ($id) {
			return __('Edit verification field', 'pn');
		} else {
			return __('Add verification field' ,'pn');
		}
...
function def_premium_action_all_add_usfield() {
	...
	if ($data_id) {	
		...
		if ($res['ind']) {
			...
			$res_errors = _debug_table_from_db($result, 'uv_field', $array);
		...
		if ($res['ind']) {
			$result = $wpdb->insert($wpdb->prefix . 'uv_field', $array);
