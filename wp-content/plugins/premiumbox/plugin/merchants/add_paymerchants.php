...
if (is_admin()) {
	...
	add_filter('pn_adminpage_title_pn_paymerchants_add', 'def_adminpage_title_pn_paymerchants_add');
	function def_adminpage_title_pn_paymerchants_add() {
		global $db_data, $wpdb;
		...
		if ($item_id) {
			$db_data = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "exts WHERE id = '$item_id' AND ext_type = 'paymerchants'");
			if (isset($db_data->id)) {
				$data_id = $db_data->id;
			}	
		}		
		...
		if ($data_id) {
			return __('Edit auto payout', 'pn');
		} else {
			return __('Add auto payout', 'pn');
		}
	}

	add_action('pn_adminpage_content_pn_paymerchants_add', 'def_adminpage_content_pn_paymerchants_add');
	function def_adminpage_content_pn_paymerchants_add() {
		...
		if ($data_id) {
			$title = __('Edit auto payout', 'pn') . ' "' . is_isset($db_data, 'ext_title') . '"';
		} else {
			$title = __('Add auto payout', 'pn');
		}
		...
		if ($data_id) {
			$back_menu['add'] = array(
				'link' => admin_url('admin.php?page=pn_paymerchants_add'),
				'title' => __('Add new','pn')
			);	
		}
		...
		$warn_temp .= $form->get_warning(sprintf(__('Do not use automatic payouts if it not urgent. Developer is not responsible for the safety of currency on your accounts. Read more here <a href="%s">here</a>.', 'pn'), 'https://premium.gitbook.io/main/preduprezhdenie-auto/'));
		$warn_temp .= '</div>';
		...
		$options['ext_status'] = array(
			'view' => 'select',
			'title' => __('Status', 'pn'),
			'options' => array('1' => __('active auto payout', 'pn'), '0' => __('inactive auto payout', 'pn')),
		...
		if ($now_script) {
			...
			$options = apply_filters('ext_paymerchants_data', $options, $now_script, $db_data->ext_key);
			...
			$options = paymerchants_setting_list($data, $db_data, 1);

			if (count($options) > 2) {
				do_action('before_paymerchants_admin', $now_script, $data, $db_data->ext_key);
...
function def_premium_action_pn_paymerchants_settings() {	
	...
	if ($data_id > 0) {
		$db_data = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "exts WHERE id = '$data_id' AND ext_type = 'paymerchants'");
		if (isset($db_data->id)) {
			$options = paymerchants_setting_list('', $db_data, 0);
			...
			$merch_data = apply_filters('_paymerchants_ext_options_array', $merch_data, $db_data);
			$array = array();
	...
	do_action('_paymerchants_options_post');		
...
add_action('premium_action_pn_paymerchants_add', 'def_premium_action_pn_paymerchants_add');
function def_premium_action_pn_paymerchants_add() {
	...
	$last_data = '';
	if ($data_id > 0) {
		$last_data = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "exts WHERE id = '$data_id' AND ext_type = 'paymerchants'");
	...
	if (isset($last_data->id)) {
		...
		$ext_key = _ext_set_key($script, 'paymerchants', $data_id);
	}		
	...
	if ($data_id) {
		...
		if (1 == $status) {
			do_action('ext_paymerchants_active_' . $script, $array['ext_key']);
			do_action('ext_paymerchants_active', $script, $array['ext_key']);	
		} else {
			do_action('ext_paymerchants_deactive_' . $script, $array['ext_key']);
			do_action('ext_paymerchants_deactive', $script, $array['ext_key']);	
		}	
	}

	$url = admin_url('admin.php?page=pn_paymerchants_add&item_id=' . $data_id . '&reply=true');
