...
jQuery(function ($) {
    ...
    $.fn.JsWindow = function (method, params) {
        ...
        if (!id.length) id = Math.floor(Math.random() * (100000 - 1 + 1)) + 1;

        var window_class = $.trim(options['window_class']);
        var insert_div = $.trim(options['insert_div']);
        var insert_obj = insert_div.length ? $(insert_div) : $('body');
        ...
        if (method === 'show') {
            ...
            let scroll_html = scrollContent.length ? `<div class="standart_window_scrollcontent">${scrollContent}</div>` : '';
            let button_html = enable_button ? `<div class="standart_window_submit"><input type="submit" class="standart_window_button ${button_class}" name="" value="${button_title}" /></div>` : '';
                    ...
                    <div class="standart_windowins">
                        <div class="standart_window_ins">
                            <div class="standart_window_title">
                                ...
                                <div class="standart_window_title_ins">${title}</div>
                            </div>
            ...
            $('#techwindow_' + id).find('img').each(function () {
                if (this.complete) {
                    create_position_window();
                } else {
                    $(this).one('load', create_position_window);
                    $(this).one('error', create_position_window);
                }
            });
            ...
            if (out_close) {
                $(document).on('click.outsideWindow', function (e) {
                    if (!$(e.target).closest('.standart_window_ins').length) {
                        $('.js_window_close').trigger('click');
                        $(document).off('click.outsideWindow');
                    }
                });
            }
        ...
        $(window).on('scroll resize', create_position_window);
