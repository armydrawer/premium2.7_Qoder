...
if (!class_exists('merchant_aipay')) {
    class merchant_aipay extends Ext_Merchant_Premiumbox {
        private array $m_types;
        private array $disable_opts = ['ext_success_url', 'ext_fail_url', 'note', 'check_api', 'check', 'invalid_ctype'];
        private array $sum_format = [false, 2, 'down'];
        private array $tx_statuses = [
            'realpay' => ['ORDER_SUCCESS'],
        ];
        ...
        function options($options, $data, $id, $place) {
            ...
            if (1 == $place && 'success' == $this->settingtext('success', $id)) {
                ...
                $api = new M_AIPAY($this->name, $id, $m_define, $m_data);
        ...
        private function init_mypaid($m_id, $pay_sum, $direction, $m_defin, $m_data) {
            ...
            $api = new M_AIPAY($this->name, $m_id, $m_defin, $m_data);

            $data = [
                'message_id' => "m_{$bids_data->id}",
                'transactionAmount' => $pay_sum,
                'callbackUrl' => apply_filters('custom_url', get_mlink("{$m_id}_webhook" . hash_url($m_id)), 'webhook', $this->name, $m_id),
                'client' => [
                    'name' => $fullname,
                    'phone' => $phone,
                    //'bank' => '',
            ...
            if (1 == $pm) {
                $data['bankAccFlag'] = true;
            } elseif (2 == $pm) {
                $data['bankTelefonFlag'] = true;
            }

            $tx = $api->create_tx($data)['pd'];

            $tx_id = pn_strip_input(is_isset($tx, 'docId'));
            $tx_to_account = pn_strip_input(is_isset($tx, 'cardNumber') ?: is_isset($tx, 'accNumber') ?: is_isset($tx, 'phone_number'));
            $tx_to_account = (str_starts_with($tx_to_account, '+') || is_isset($tx, 'phone_number') ? '+' : '') . preg_replace('/\D/', '', $tx_to_account);
            ...
            $dt_data = array_filter([
                'cardholder' => pn_strip_input(is_isset($tx, 'cardHolder')),
                'bank_name' => pn_strip_input(is_isset($tx, 'pay_system')),
            ]);
        ...
        private function _payment_check($place, $m_id, $m_define, $m_data, $tx_info = false) {
            ...
            try {
                ...
                foreach ($bids as $bid) {
                    ...
                    $tx_amount = $this->_sum_format(is_isset($tx, 'notification_transactionAmount'), $m_id);
                    $tx_currency = '';
                    $tx_hash = '';
                    $tx_purse_from = '';
                    $tx_purse = '';
...
new merchant_aipay(__FILE__, 'AI-pay');
