...
/*
title: [en_US:]TrustixPay[:en_US][ru_RU:]TrustixPay[:ru_RU]
description: [en_US:]TrustixPay merchant[:en_US][ru_RU:]мерчант TrustixPay[:ru_RU]
version: 2.7.0
*/

if (!class_exists('Ext_Merchant_Premiumbox')) return;

if (!class_exists('merchant_trustixpay')) {
    class merchant_trustixpay extends Ext_Merchant_Premiumbox {
        private array $m_types;
        private array $disable_opts = ['note', 'check_api', 'check', 'invalid_ctype'];
        private array $sum_format = [2, 'down'];
        private array $tx_statuses = [
            'realpay' => ['SUCCESS', 'PAID'],
        ];
        ...
        function options($options, $data, $id, $place) {
            ...
            if (1 == $place && 'success' == $this->settingtext('success', $id)) {
                ...
                $api = new M_TRUSTIXPAY($this->name, $id, $m_define, $m_data);

                $r = $api->get_tx('00000000-0000-0000-0000-000000000000');

                if (isset($r['json']['message']) && str_contains(strtolower($r['json']['message']), 'invoice not found')) {
        ...
        function init_link($m_id, $pay_sum, $direction, $m_defin, $m_data) {
            ...
            $api = new M_TRUSTIXPAY($this->name, $m_id, $m_defin, $m_data);

            $data = [
                'invoice_id' => "m_{$bids_data->id}",
                'amount' => $pay_sum,
                'requisite_type' => 'crossBorderTransfer',
                'PaymentSuccessURL' => apply_filters('custom_url', get_mlink("{$m_id}_success") . "?order_id={$bids_data->id}", 'success', $this->name, $m_id),
                'PaymentFailURL' => apply_filters('custom_url', get_mlink("{$m_id}_fail") . "?order_id={$bids_data->id}", 'fail', $this->name, $m_id),
                'payment_page' => 1,
            ];
            ...
            $tx_url = !empty($tx['requisite']['link']) ? pn_strip_input($tx['requisite']['link']) : null;
        ...
        function webhook($m_id, $m_define, $m_data) {
            ...
            $tx_id = !empty($data['p_id']) ? pn_strip_input($data['p_id']) : null;
            $tx_status = !empty($data['p_status']) ? mb_strtoupper($data['p_status']) : null;
        ...
        private function _payment_check($place, $m_id, $m_define, $m_data, $tx_info = false) {
            ...
            try {
                ...
                $api = new M_TRUSTIXPAY($this->name, $m_id, $m_define, $m_data);
                ...
                foreach ($bids as $bid) {
                    ...
                    $tx_amount = !empty($tx['amount_rub']) ? $this->_sum_format($tx['amount_rub'], $m_id) : null;
                    $tx_currency = '';
                    $tx_hash = '';
                    $tx_purse_from = '';
                    $tx_purse = '';
...
new merchant_trustixpay(__FILE__, 'TrustixPay');
