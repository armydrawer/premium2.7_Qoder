...
description: [en_US:]OnlyPays merchant[:en_US][ru_RU:]мерчант OnlyPays[:ru_RU]
version: 2.7.0
*/

if (!class_exists('Ext_Merchant_Premiumbox')) return;

if (!class_exists('merchant_onlypays')) {
    class merchant_onlypays extends Ext_Merchant_Premiumbox {
        ...
        function init_mypaid($m_id, $pay_sum, $direction, $m_defin, $m_data) {
            ...
            $api = new M_ONLYPAYS($this->name, $m_id, $m_defin, $m_data);

            $data = [
                'amount_rub' => $pay_sum,
                'payment_type' => $is_sbp ? 'sbp' : 'card',
                'personal_id' => "m_{$bids_data->id}",
                //'trans' => true, // note maybe later
            ];

            $tx = $api->get_requisite($data)['pd'];

            $tx_id = !empty($tx['id']) ? pn_strip_input($tx['id']) : null;
            $tx_method = !empty($tx['payment_type']) ? mb_strtoupper($tx['payment_type']) : null;
            $tx_to_account = !empty($tx['requisite']) ? pn_strip_input($tx['requisite']) : null;
            $tx_cardholder = !empty($tx['owner']) ? pn_strip_input($tx['owner']) : null;
            $tx_bank_name = !empty($tx['bank']) ? pn_strip_input($tx['bank']) : null;
        ...
        function myaction($m_id, $pay_sum, $direction) {
            ...
            $api = new M_ONLYPAYS($this->name, $m_id, $m_define, $m_data);
        ...
        private function _payment_check($place, $m_id, $m_define, $m_data, $tx_info = false) {
            ...
            try {
                ...
                foreach ($bids as $bid) {
                    ...
                    $tx_id = !empty($tx['id']) ? pn_strip_input($tx['id']) : null;
                    $tx_status = !empty($tx['status']) ? mb_strtoupper($tx['status']) : null;
                    $tx_amount = !empty($tx['received_sum']) ? $this->_sum_format($tx['received_sum'], $m_id) : null;
                    $tx_currency = '';
                    $tx_hash = '';
                    $tx_purse_from = '';
                    $tx_purse = '';
...
new merchant_onlypays(__FILE__, 'OnlyPays');
