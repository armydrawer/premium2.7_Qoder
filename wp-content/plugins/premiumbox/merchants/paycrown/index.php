...
/*
title: [en_US:]PayCrown[:en_US][ru_RU:]PayCrown[:ru_RU]
description: [en_US:]PayCrown merchant[:en_US][ru_RU:]мерчант PayCrown[:ru_RU]
version: 2.7.0
*/

if (!class_exists('Ext_Merchant_Premiumbox')) return;

if (!class_exists('merchant_paycrown')) {
    class merchant_paycrown extends Ext_Merchant_Premiumbox {
        ...
        function settings_list() {
            return [['MERCHANT_ID', 'API_KEY', 'SECRET_KEY']];
        }
        ...
        function options($options, $data, $id, $place) {
            ...
            if (1 == $place && 'success' == $this->settingtext('success', $id)) {
                ...
                $api = new M_PAYCROWN($this->name, $id, $m_define, $m_data);
        ...
        function init_mypaid($m_id, $pay_sum, $direction, $m_defin, $m_data) {
            ...
            $api = new M_PAYCROWN($this->name, $m_id, $m_defin, $m_data);

            $data = [
                'amount' => $pay_sum,
                'currency' => mb_strtolower($currency_code_give),
                'payment_type' => $pm,
                'external_id' => "m_{$bids_data->id}",
                'callback_url' => apply_filters('custom_url', get_mlink("{$m_id}_webhook" . hash_url($m_id)), 'webhook', $this->name, $m_id),
            ];
            ...
            $tx_to_account = !empty($tx['requisites']['requisites']) ? pn_strip_input($tx['requisites']['requisites']) : null;
            $tx_cardholder = !empty($tx['requisites']['name']) ? pn_strip_input($tx['requisites']['name']) : null;
            $tx_bank_name = !empty($tx['requisites']['bank']) ? pn_strip_input($tx['requisites']['bank']) : null;
        ...
        function myaction($m_id, $pay_sum, $direction) {
            ...
            $api = new M_PAYCROWN($this->name, $m_id, $m_define, $m_data);
...
new merchant_paycrown(__FILE__, 'PayCrown');
