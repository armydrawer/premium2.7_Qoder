...
/*
title: [en_US:]PayMatrix[:en_US][ru_RU:]PayMatrix[:ru_RU]
description: [en_US:]Merchant PayMatrix[:en_US][ru_RU:]Мерчант PayMatrix[:ru_RU]
version: 2.7.1
*/

if (!class_exists('Ext_Merchant_Premiumbox')) return;

if (!class_exists('merchant_paymatrix')) {
    class merchant_paymatrix extends Ext_Merchant_Premiumbox {
        ...
        function options($options, $data, $m_id, $place) {
            ...
            $payment_methods = array(
                'RUB:::SBP' => '[RUB] SBP',
                'RUB:::Cards' => '[RUB] Cards',
                //'RUB:::SBP_aquiring' => '[RUB] SBP_aquiring',
                'RUB:::SBP_Transgran' => '[RUB] SBP_Transgran',
                'RUB:::Cards_Transgran' => '[RUB] Cards_Transgran',
                'RUB:::mobile_MegaFone_Tjs' => '[RUB] mobile_MegaFone_Tjs',

                'UZS:::Uzcard' => '[UZS] Uzcard',
                'UZS:::Humo' => '[UZS] Humo',
                'UZS:::Payme' => '[UZS] Payme',
                'UZS:::Click' => '[UZS] Click',
                //'UZS:::QR_Click' => '[UZS] QR_Click',

                'AZN:::M10' => '[AZN] M10',
                'AZN:::Kapital' => '[AZN] Kapital',
                'AZN:::Access' => '[AZN] Access',
            );
        ...
        function init($m_id, $pay_sum, $direction, $m_defin, $m_data) {
            ...
            if (!$bids_data->to_account) {
                ...
                $pm_type = is_isset($pm_exp, 1);

                $arr = array();

                $api = new M_PAYMATRIX($this->name, $m_id, is_isset($m_defin, 'API_KEY'));

                $data = array(
                    'amount' => $pay_sum,
                    'currency_code' => $pm_currency,
                    'payment_method' => $pm_type,
                    'customer_id' => $bids_data->user_id,
                    'merchant_payment_id' => 'm_' . $bids_data->id,
                );
                $r = $api->trade($data);

                $is_sbp = stripos($pm_type, 'sbp') !== false;

                $to_account = '';
                $dest_tag = '';
                $trans_in = '';

                if (isset($r['id'], $r['pan'], $r['seller_name'])) {
                    $to_account = preg_replace('/\D/', '', $r['pan']);
                    ...
                    $dest_tag_arr = array_values(array_filter(array(
                        'card_holder' => !empty($r['seller_name']) ? pn_strip_input($r['seller_name']) : null,
                        'bank' => !empty($r['SupportBankNative']) ? pn_strip_input(implode(', ', $r['SupportBankNative'])) : null,
                    )));
        ...
        function myaction($m_id, $pay_sum, $direction) {
            ...
            if ($script and $script == $this->name) {
                ...
                if ($bids_data->trans_in) {
                    ...
                    $api = new M_PAYMATRIX($this->name, $m_id, is_isset($m_defin, 'API_KEY'));
                    $r = $api->get_trade($bids_data->trans_in);

                    if (isset($r['id'])) {

                        $api->trade_confirm($bids_data->trans_in);
        ...
        function merch_cron($m_id, $m_defin, $m_data, $order_id = '') {
            ...
            try {
                ...
                foreach ($items as $item) {
                    ...
                    $tx_sum = is_sum($tx['cost'], 4);

                    if (in_array($tx_status, array('CLOSED'))) {
                        ...
                        $bid_sum = is_sum($data['pay_sum'], 4);
                        $bid_corr_sum = apply_filters('merchant_bid_sum', $bid_sum, $bid_m_id);
                        $bid_corr_sum = is_sum($bid_corr_sum, 4);
...
new merchant_paymatrix(__FILE__, 'PayMatrix');
