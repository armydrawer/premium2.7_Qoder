...
class M_NICEPAY {
    ...
    function paymentUrl($data) {
        ...
        return $this->request('POST', '/public/api/payment', ['json' => $data]);
    }

    function h2hOneRequestPayment($data) {

        return $this->request('POST', '/public/api/h2hOneRequestPayment', ['json' => $data]);
    }

    function h2hConfirmPaid($id) {

        return $this->request('POST', '/public/api/h2hConfirmPaid', ['json' => ['payment' => $id]]);
    }

    function h2hPaymentInfo($id) {

        $r = $this->request('POST', '/public/api/h2hPaymentInfo', ['json' => ['payment' => $id]]);

        $r['pd'] = !empty($r['json']['data']['paymentId']) ? $r['json']['data'] : [];
    ...
    function paymentMethods($add_new = '') {

        $data = [];
        $r = $this->request('GET', '/public/api/paymentMethods');
