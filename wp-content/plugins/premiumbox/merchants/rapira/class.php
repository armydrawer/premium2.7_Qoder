...
if (!class_exists('Rapira')) {
	class Rapira {
		...
		function set_token() {
			...
			if (!$token) {
				...
				if ($ch = curl_init()) {
					...
					do_action('save_merchant_error', $this->m_name, $this->m_id, 'generate_jwt', $headers, $json_data, $result, $err);
		...
		function get_history_deposits($limit) {
			...
			$limit = intval($limit);
			$post = array( "pageNo" => 0, "pageSize" => $limit);
			$res = $this->request('/open/deposit/records', $post, "multipart/form-data");
			$trans = array();
			if (is_array($res) and !isset($res['error']) and 500 != $res['code'] and isset($res['data']['content'])) {
				$trans = $res['data']['content'];
			}
		...
		function request($method, $post = array(), $content_type = "application/json") {
			...
			if ($ch = curl_init()) {
				...
				if (is_array($post) and count($post) > 0) {
					if ("application/json" == $content_type) { $post = json_encode($post); }
					curl_setopt($ch, CURLOPT_POST, true);
				...
				do_action('save_merchant_error', $this->m_name, $this->m_id, $method, $headers, $json_data, $result, $err);
