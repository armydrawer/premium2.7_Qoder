...
/*
Documentation: https://pspware.gitbook.io/pspware
*/

class M_PSPWARE {
    private array $cfg = [
        'base_url' => 'https://api.pspware.space',
        'MERCHANT_ID' => null,
        'API_KEY' => null,
        ////////////////////////////////////////
        'm_name' => null,
    ...
    function get_merchant() {
        $options = [
            'params' => [
                'sign' => hash('sha256', "{$this->cfg['MERCHANT_ID']}:{$this->cfg['API_KEY']}"),
            ],
            'secure_response' => ['api_key'],
        ];

        return $this->request('GET', "/payphoria/merchant/api/v1/merchants/{$this->cfg['MERCHANT_ID']}", $options);
    }

    function order($data) {

        $sum = ctype_digit(strval($data['sum'])) ? "{$data['sum']}.0" : $data['sum'];

        $data = array_merge($data, [
            'merchant_id' => $this->cfg['MERCHANT_ID'],
            'signature' => hash('sha256', "{$sum}:{$this->cfg['API_KEY']}"),
        ]);

        $prev_timeout = $this->cfg['timeout'];
        $this->cfg['timeout'] = 10;
        $r = $this->request('POST', '/payphoria/merchant/api/v1/orders', ['json' => $data]);
        $this->cfg['timeout'] = $prev_timeout;
    ...
    function get_payment($id, $sum) {

        $sum = ctype_digit(strval($sum)) ? "{$sum}.0" : $sum;

        $data = [
            'sign' => hash('sha256', "{$sum}:{$this->cfg['API_KEY']}"),
        ];

        $r = $this->request('POST', "/payphoria/merchant/api/v1/orders/{$id}", ['json' => $data]);
