...
description: [en_US:]Bankoro merchant[:en_US][ru_RU:]мерчант Bankoro[:ru_RU]
version: 2.7.0
*/

if (!class_exists('Ext_Merchant_Premiumbox')) return;

if (!class_exists('merchant_bankoro')) {
    class merchant_bankoro extends Ext_Merchant_Premiumbox {
        private array $m_types;
        private array $disable_opts = ['ext_success_url', 'ext_fail_url', 'check_api'];
        ...
        function options($options, $data, $id, $place) {
            ...
            if (1 == $place && 'success' == $this->settingtext('success', $id)) {
                ...
                $api = new M_BANKORO($this->name, $id, $m_define, $m_data);
        ...
        private function init_address($m_id, $pay_sum, $direction, $m_defin, $m_data) {
            global $bids_data;
            ...
            $api = new M_BANKORO($this->name, $m_id, $m_defin, $m_data);
            ...
            $tx_to_account = pn_strip_input(is_isset($tx, 'value'));
            $tx_dest_tag = '';
        ...
        private function webhook($m_id, $m_define, $m_data) {
            ...
            if (count(array_filter($checked_fields)) !== count($checked_fields) || 'DEPOSIT' != $tx_dir || empty($this->tx_statuses[$tx_status])) {
        ...
        private function _payment_check($place, $m_id, $m_define, $m_data, $tx_info = false) {
            ...
            try {
                ...
                $api = new M_BANKORO($this->name, $m_id, $m_define, $m_data);
                ...
                foreach ($bids as $bid) {
                    ...
                    $tx_currency = pn_strip_input(is_isset(is_isset($tx, 'symbol'), 'short_name'));
                    $tx_hash = pn_strip_input(is_isset(is_isset($tx, 'blockchain_data'), 'hash'));
                    $tx_purse_from = pn_strip_input(is_isset(is_isset($tx, 'blockchain_data'), 'from_address'));
                    $tx_purse = pn_strip_input(is_isset(is_isset($tx, 'blockchain_data'), 'to_address'));
...
new merchant_bankoro(__FILE__, 'Bankoro');
