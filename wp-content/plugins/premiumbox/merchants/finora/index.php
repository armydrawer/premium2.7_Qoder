...
/*
title: [en_US:]Finora[:en_US][ru_RU:]Finora[:ru_RU]
description: [en_US:]Merchant Finora[:en_US][ru_RU:]Мерчант Finora[:ru_RU]
version: 2.7.0
*/

if (!class_exists('Ext_Merchant_Premiumbox')) return;

if (!class_exists('merchant_finora')) {
    class merchant_finora extends Ext_Merchant_Premiumbox {
        ...
        function init($m_id, $pay_sum, $direction, $m_defin, $m_data) {
            ...
            if (!$bids_data->to_account) {
                ...
                $api = new M_FINORA($this->name, $m_id, is_isset($m_defin, 'API_KEY'));
...
new merchant_finora(__FILE__, 'Finora');
