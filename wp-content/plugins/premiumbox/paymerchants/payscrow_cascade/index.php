...
description: [en_US:]Payscrow Cascade automatic payouts[:en_US][ru_RU:]авто выплаты Payscrow Cascade[:ru_RU]
version: 2.7.0
*/

if (!class_exists('Ext_AutoPayut_Premiumbox')) return;

if (!class_exists('paymerchant_payscrow_c')) {
    class paymerchant_payscrow_c extends Ext_AutoPayut_Premiumbox {
        ...
        function options($options, $data, $m_id, $place) {
            ...
            if (1 == $place and is_isset($m_defin, 'API_KEY')) {
                ...
                $api = new PAY_PAYSCROW_C($this->name, $m_id, is_isset($m_defin, 'API_KEY'));

                $r = $api->payment_methods();
                unset($r['2ec6dbd6-49a5-45d0-bd6d-b0134ee4639a']); // SBP
                ...
                foreach ($r as $method) {
                    ...
                    if (!in_array($method_type, ['SBP', 'BankCard'])) continue;
        ...
        function do_auto_payouts($error, $pay_error, $m_id, $item, $place, $direction_data, $paymerch_data, $unmetas, $modul_place, $direction, $test, $m_defin) {
            ...
            if (0 == count($error)) {
                ...
                if ($result) {
                    try {
                        $fullname = pn_strip_input(is_isset($unmetas, 'get_cardholder') ?: is_isset($unmetas, 'cardholder') ?: implode(' ', array_filter([$item->last_name, $item->first_name, $item->second_name])));
...
new paymerchant_payscrow_c(__FILE__, 'Payscrow Cascade');