...
description: [en_US:]OptiMoney automatic payouts[:en_US][ru_RU:]авто выплаты OptiMoney[:ru_RU]
version: 2.7.1
*/

if (!class_exists('Ext_AutoPayut_Premiumbox')) return;

if (!class_exists('paymerchant_optimoney')) {
    class paymerchant_optimoney extends Ext_AutoPayut_Premiumbox {
        private array $disable_opts = ['ext_webhook_url', 'checkpay'];
        ...
        function options($options, $data, $id, $place) {
            ...
            if (1 == $place && 'success' == $this->settingtext('success', $id)) {
                ...
                $api = new P_OPTIMONEY($this->name, $id, $m_define, $m_data);

                $r = $api->payment_methods_list();
        ...
        function update_reserve($code, $m_id, $m_defin) {
            ...
            try {
                ...
                $api = new P_OPTIMONEY($this->name, $m_id, $m_defin, $m_data);
        ...
        function do_auto_payouts($error, $pay_error, $m_id, $item, $place, $direction_data, $paymerch_data, $unmetas, $modul_place, $direction, $test, $m_defin) {
            ...
            if (!$error) {
                try {
                    $api = new P_OPTIMONEY($this->name, $m_id, $m_defin, $paymerch_data);

                    $data = [
                        'from_wallet_number' => $pm,
                        'to_wallet_number' => $account,
                        'amount' => $pay_sum,
            ...
            if ($error) {
                ...
                $tx_amount = $this->_sum_format($tx['amount_without_fee'] ?? '' ?: '', $m_id);
                $tx_hash = pn_strip_input($tx['transaction_number'] ?? '' ?: '');
        ...
        private function _payment_check($place, $m_id, $m_define, $m_data, $tx_info = false) {
            ...
            try {
                ...
                $api = new P_OPTIMONEY($this->name, $m_id, $m_define, $m_data);
...
new paymerchant_optimoney(__FILE__, 'OptiMoney');
