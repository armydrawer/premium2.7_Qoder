...
class P_CRYPTOCASH_C {
    ...
    function balance($force_refresh = false) {
        ...
        $data = [];
        $r = $this->request('POST', "/merchant/api/v1/balance/retrieve/");

        if (!empty($r['json']['data']['item'])) {
            foreach ($r['json']['data']['item'] as $k => $val) {
                $_k = mb_strtolower(str_ireplace('amount', '_usdt', pn_strip_input($k)));
                $_val = is_sum($val);
    ...
    function buy($data) {
        ...
        return $this->request('POST', "/merchant/api/v1/balance/actions/buy/crypto/", ['json' => $data]);
    }
