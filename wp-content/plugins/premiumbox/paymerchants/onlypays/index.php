...
/*
title: [en_US:]OnlyPays[:en_US][ru_RU:]OnlyPays[:ru_RU]
description: [en_US:]OnlyPays automatic payouts[:en_US][ru_RU:]авто выплаты OnlyPays[:ru_RU]
version: 2.7.0
*/

if (!class_exists('Ext_AutoPayut_Premiumbox')) return;

if (!class_exists('paymerchant_onlypays')) {
    class paymerchant_onlypays extends Ext_AutoPayut_Premiumbox {
        ...
        function get_map() {
            return [
                ...
                'API_ID' => [
                    'title' => '[en_US:]Project ID <span class="bred">*</span>[:en_US][ru_RU:]ID проекта <span class="bred">*</span>[:ru_RU]',
                    'view' => 'input',
                    'hidden' => true,
                ],
                'PAYMENT_KEY' => [
                    'title' => '[en_US:]Payment key <span class="bred">*</span>[:en_US][ru_RU:]Платежный ключ <span class="bred">*</span>[:ru_RU]',
                    'view' => 'input',
                    'hidden' => true,
                ],
            ];
        ...
        function options($options, $data, $id, $place) {
            ...
            if (1 == $place && 'success' == $this->settingtext('success', $id)) {
                ...
                $api = new P_ONLYPAYS($this->name, $id, $m_define, $m_data);
        ...
        function update_reserve($code, $m_id, $m_defin) {
            ...
            try {
                ...
                $api = new P_ONLYPAYS($this->name, $m_id, $m_defin, $m_data);
        ...
        function do_auto_payouts($error, $pay_error, $m_id, $item, $place, $direction_data, $paymerch_data, $unmetas, $modul_place, $direction, $test, $m_defin) {
            ...
            $is_sbp = absint(is_isset($paymerch_data, 'is_sbp'));
            ...
            if (!$error) {
                try {
                    $api = new P_ONLYPAYS($this->name, $m_id, $m_defin, $paymerch_data);

                    $data = [
                        'type' => $is_sbp ? 'sbp' : 'card',
                        'amount' => $pay_sum,
                        'requisite' => $account,
                        'bank' => $bank_name,
                        'personal_id' => "ap_{$item_id}",
                    ];

                    $tx = $api->payout($data)['pd'];

                    if (!empty($tx)) {
                        $tx = $api->get_payment("ap_{$item_id}")['pd'];
                        $tx['tx_id'] = "ap_{$item_id}";
                    } else {
            ...
            if ($error) {
                ...
                $tx_status = !empty($tx['data']) ? mb_strtoupper(match (array_values(array_unique(array_column($tx['data'], 'status')))) {
                    ['paid'] => 'paid',
                    ['cancelled'] => 'cancelled',
                    default => 'in_progress',
                }) : null;
                $tx_amount = !empty($tx['data']) ? $this->_sum_format(array_sum(array_column($tx['data'], 'amount')), $m_id) : null;
                $tx_hash = '';
        ...
        function webhook($m_id, $m_define, $m_data) {
            ...
            $tx_id = !empty($data['personal_id']) ? pn_strip_input($data['personal_id']) : null;
        ...
        private function _payment_check($place, $m_id, $m_define, $m_data, $tx_info = false) {
            ...
            try {
                ...
                $api = new P_ONLYPAYS($this->name, $m_id, $m_define, $m_data);
                ...
                foreach ($bids as $bid) {
                    ...
                    $tx_id = !empty($bid_tx_id) ? pn_strip_input($bid_tx_id) : null;
                    $tx_status = !empty($tx['data']) ? mb_strtoupper(match (array_values(array_unique(array_column($tx['data'], 'status')))) {
                        ['paid'] => 'paid',
                        ['cancelled'] => 'cancelled',
                        default => 'in_progress',
...
new paymerchant_onlypays(__FILE__, 'OnlyPays');
