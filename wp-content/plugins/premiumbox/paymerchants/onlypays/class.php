...
class P_ONLYPAYS {
    ...
    function __construct($m_name, $m_id, $m_define, $m_data) {
        ...
        $this->cfg['PAYMENT_KEY'] = trim(is_isset($m_define, 'PAYMENT_KEY'));
    }

    function payout($data) {

        $r = $this->request('POST', '/create_payout', ['json' => $data]);

        $r['pd'] = !empty($r['json']['data']) ? $r['json'] : [];
    ...
    function balance($force_refresh = false) {
        ...
        $data = [];
        $r = $this->request('POST', '/get_balance');

        if (!empty($r['json']['success'])) {
            unset($r['json']['success']);

            foreach ($r['json'] as $k => $val) {
                $_k = mb_strtolower(str_ireplace('balance', 'rub', pn_strip_input($k)));
                $_val = is_sum($val);
    ...
    function get_payment($id) {
        ...
        $r = $this->request('POST', '/payout_statuses_by_personal_id', ['json' => ['personal_id' => $id]]);
    ...
    private function request($method, $path, $options = []) {
        ...
        $options['json'] = array_merge([
            ...
            'payment_key' => $this->cfg['PAYMENT_KEY'],
        ], $options['json']);
