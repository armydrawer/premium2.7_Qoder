...
if (!class_exists('AP_UTOPIA_C')) {
    class AP_UTOPIA_C {
        ...
        function getBalance($currency) {
            ...
            return $this->request('getBalance', $data);
        }

        function createVoucher($data) {

            if (isset($data['currency'])) {
                $data['currency'] = ('UUSD' == $data['currency'] ? 'USD' : $data['currency']);
            }

            $r = $this->request('createVoucher', $data);
            if (isset($r['result']) and '{' == $r['result'][0]) {
                unset($r['result']);
            }
