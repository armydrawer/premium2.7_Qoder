<?php

/*
https://rapira.readme.io/reference/intro
*/

if (!class_exists('JWT')) {
    class JWT {
        function __construct() {

        }

        function encode($payload, $key) {
            $algo = 'RS256';
            $header = array('typ' => 'JWT', 'alg' => $algo);

            $segments = array();
            $segments[] = $this->urlsafeB64Encode($this->jsonEncode($header));
            $segments[] = $this->urlsafeB64Encode($this->jsonEncode($payload));
            $signing_input = implode('.', $segments);

            $signature = $this->sign($signing_input, $key);
            $segments[] = $this->urlsafeB64Encode($signature);

            return implode('.', $segments);
        }

        function sign($payload, $key) {
            $passphrase = '';

            $algo = OPENSSL_ALGO_SHA256;
            $key_type = OPENSSL_KEYTYPE_RSA;

            $privateKey = openssl_pkey_get_private($key, $passphrase);

            if (is_bool($privateKey)) {
                $error = openssl_error_string();
                throw new Exception($error);
            }

            $details = openssl_pkey_get_details($privateKey);

            if (!array_key_exists('key', $details) || $details['type'] !== $key_type) {
                throw new Exception("Invalid key provided");
            }

            $signature = '';

            if (!openssl_sign($payload, $signature, $privateKey, $algo)) {
                $error = openssl_error_string();
                throw new Exception($error);
            }

            return $signature;
        }

        function jsonDecode($input) {
            $obj = json_decode($input);
            if (function_exists('json_last_error') && $errno = json_last_error()) {
                $this->_handleJsonError($errno);
            } else if ($obj === null && $input !== 'null') {
                throw new Exception('Null result with non-null input');
            }
            return $obj;
        }

        function jsonEncode($input) {
            $json = json_encode($input);
            if (function_exists('json_last_error') && $errno = json_last_error()) {
                $this->_handleJsonError($errno);
            } else if ($json === 'null' && $input !== null) {
                throw new Exception('Null result with non-null input');
            }
            return $json;
        }

        function urlsafeB64Decode($input) {
            $remainder = strlen($input) % 4;
            if ($remainder) {
                $padlen = 4 - $remainder;
                $input .= str_repeat('=', $padlen);
            }
            return base64_decode(strtr($input, '-_', '+/'));
        }

        function urlsafeB64Encode($input) {
            return str_replace('=', '', strtr(base64_encode($input), '+/', '-_'));
        }

        function _handleJsonError($errno) {
            $messages = array(
                JSON_ERROR_DEPTH => 'Maximum stack depth exceeded',
                JSON_ERROR_CTRL_CHAR => 'Unexpected control character found',
                JSON_ERROR_SYNTAX => 'Syntax error, malformed JSON'
            );
            throw new Exception(
                isset($messages[$errno])
                    ? $messages[$errno]
                    : 'Unknown JSON error: ' . $errno
            );
        }
    }
}

if (!class_exists('AP_Rapira')) {
    class AP_Rapira {

        private $m_name = "";
        private $m_id = "";
        private $token = "";
        public $private_key = '';
        public $uid = '';
        public $hosts = '';
        public $host_type = 0;

        function __construct($m_name, $m_id, $private_key = '', $uid = '', $host_type = 0) {

            $this->hosts = array(
                '0' => array(
                    'api' => 'api.rapira.net',
                ),
                '1' => array(
                    'api' => 'api.rapira.org',
                ),
            );
            $this->host_type = intval($host_type);
            $this->m_name = trim($m_name);
            $this->m_id = trim($m_id);
            $this->private_key = trim($private_key);
            $this->uid = trim($uid);

            if ($this->private_key and $this->uid) {
                $this->set_token();
            }

        }

        function set_token() {

            $token = trim($this->token);
            if (!$token) {

                $request = array('exp' => time() + 3600, 'jti' => bin2hex(random_bytes(12)));
                $class = new JWT();
                $payload = $class->encode($request, base64_decode($this->private_key, true));
                $post_data = array('kid' => $this->uid, 'jwt_token' => $payload);

                $json_data = json_encode($post_data);

                $headers = array(
                    'Content-Type: application/json',
                );

                $host = $this->hosts[$this->host_type]['api'];

                if ($ch = curl_init()) {

                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:65.0) Gecko/20100101 Firefox/65.0');
                    curl_setopt($ch, CURLOPT_URL, "https://{$host}/open/generate_jwt");
                    curl_setopt($ch, CURLOPT_POST, true);
                    curl_setopt($ch, CURLOPT_POSTFIELDS, $json_data);
                    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
                    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
                    curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
                    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, false);
                    curl_setopt($ch, CURLOPT_HEADER, false);
                    curl_setopt($ch, CURLOPT_TIMEOUT, 20);
                    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 20);
                    $ch = apply_filters('curl_ap', $ch, $this->m_name, $this->m_id);

                    $err = curl_errno($ch);
                    $result = curl_exec($ch);

                    curl_close($ch);

                    do_action('save_paymerchant_error', $this->m_name, $this->m_id, 'generate_jwt', $headers, $json_data, $result, $err);

                    $res = @json_decode($result, true);

                    if (isset($res['token'])) {
                        $this->token = $res['token'];
                    }
                }
            }
        }

        function get_coins() {

            $coins = array();
            $post = array();
            $res = $this->request('/open/wallet/balance', $post);
            if (is_array($res)) {
                foreach ($res as $coin) {
                    if (isset($coin['unit']) and $coin['unit']) {
                        $coins[] = strtoupper(pn_strip_input($coin['unit']));
                    }
                }
            }

            return array_unique($coins);
        }

        function get_balance() {

            $balance = array();
            $post = array();
            $res = $this->request('/open/wallet/balance', $post);
            if (is_array($res)) {
                foreach ($res as $coin) {
                    if (isset($coin['unit'], $coin['balance'])) {
                        $unit = strtoupper(pn_strip_input($coin['unit']));
                        $b = is_sum($coin['balance']);
                        $balance[$unit] = $b;
                    }
                }
            }

            return $balance;
        }

        function create_address($currency) {

            $currency = trim(strtolower($currency));

            $post = array(
                'currency' => $currency,
            );
            $res = $this->request('/open/deposit_address?currency=' . $currency, $post);

            $data = array();

            if (isset($res['id'], $res['address']) and !$res['address']) {
                $id = intval($res['id']);
                for ($i = 1; $i <= 5; $i++) {
                    sleep(2);

                    $res2 = $this->request('/open/deposit_address/details?id=' . $id, array());

                    if (isset($res2['address'])) {

                        $data['address'] = $res2['address'];
                        break;

                    }
                }
            } elseif (isset($res['id'], $res['address']) and $res['address']) {
                $data['address'] = $res['address'];
            }

            return $data;
        }

        function create_payout($code, $address, $amount, $nonce, $memo = '', $from = '') {

            $post = array(
                'code' => $code,
                'address' => $address,
                'amount' => $amount,
                'nonce' => time() . $nonce,
            );

            $memo = trim($memo);
            if ($memo) {
                $post['memo'] = $memo;
            }

            $from = trim($from);
            if ($from) {

                $post['conversionDetails'] = array(
                    'conversionEnabled' => true,
                    'convertFromCurrency' => $from,
                    'enoughCoinPayment' => false,
                );

            }

            return $res = $this->request('/open/withdraw/create', $post);
        }

        function get_history_payout($id) {

            $id = intval($id);
            $post = array();

            return $res = $this->request('/open/withdraw/crypto/history/' . $id, $post);
        }

        function request($method, $post = array(), $content_type = "application/json") {

            if (!$this->private_key or !$this->uid) {
                return '';
            }

            $json_data = '';

            $headers = array(
                "Content-Type: " . $content_type,
                "Authorization: Bearer {$this->token}"
            );

            $host = $this->hosts[$this->host_type]['api'];
            $url = 'https://' . $host . $method;

            if ($ch = curl_init()) {

                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:65.0) Gecko/20100101 Firefox/65.0');
                curl_setopt($ch, CURLOPT_URL, $url);

                if (is_array($post) and count($post) > 0) {
                    if ("application/json" == $content_type) {
                        $post = json_encode($post);
                    }
                    curl_setopt($ch, CURLOPT_POST, true);
                    curl_setopt($ch, CURLOPT_POSTFIELDS, $post);
                }

                $json_data = $post;

                curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
                curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
                curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
                curl_setopt($ch, CURLOPT_FOLLOWLOCATION, false);
                curl_setopt($ch, CURLOPT_HEADER, false);
                curl_setopt($ch, CURLOPT_TIMEOUT, 20);
                curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 20);
                $ch = apply_filters('curl_ap', $ch, $this->m_name, $this->m_id);

                $err = curl_errno($ch);
                $result = curl_exec($ch);

                curl_close($ch);

                do_action('save_paymerchant_error', $this->m_name, $this->m_id, $url, $headers, $json_data, $result, $err);

                $res = @json_decode($result, true);

                return $res;
            }

            return '';
        }
    }
}